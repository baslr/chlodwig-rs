  var AI8 = d((BT) => {
    var tb8 = S0(),
      r56 = xA(),
      l56 = U56(),
      Mk$ = qz(),
      ab8 = _k();
    class eb8 {
      config;
      middlewareStack = tb8.constructStack();
      initConfig;
      handlers;
      constructor(H) {
        this.config = H;
      }
      send(H, _, q) {
        let $ = typeof _ !== "function" ? _ : void 0,
          K = typeof _ === "function" ? _ : q,
          O = $ === void 0 && this.config.cacheMiddleware === !0,
          T;
        if (O) {
          if (!this.handlers) this.handlers = new WeakMap();
          let z = this.handlers;
          if (z.has(H.constructor)) T = z.get(H.constructor);
          else (T = H.resolveMiddleware(this.middlewareStack, this.config, $)), z.set(H.constructor, T);
        } else delete this.handlers, (T = H.resolveMiddleware(this.middlewareStack, this.config, $));
        if (K)
          T(H)
            .then(
              (z) => K(null, z.output),
              (z) => K(z),
            )
            .catch(() => {});
        else return T(H).then((z) => z.output);
      }
      destroy() {
        this.config?.requestHandler?.destroy?.(), delete this.handlers;
      }
    }
    var Q56 = "***SensitiveInformation***";
    function i56(H, _) {
      if (_ == null) return _;
      let q = Mk$.NormalizedSchema.of(H);
      if (q.getMergedTraits().sensitive) return Q56;
      if (q.isListSchema()) {
        if (!!q.getValueSchema().getMergedTraits().sensitive) return Q56;
      } else if (q.isMapSchema()) {
        if (!!q.getKeySchema().getMergedTraits().sensitive || !!q.getValueSchema().getMergedTraits().sensitive)
          return Q56;
      } else if (q.isStructSchema() && typeof _ === "object") {
        let $ = _,
          K = {};
        for (let [O, T] of q.structIterator()) if ($[O] != null) K[O] = i56(T, $[O]);
        return K;
      }
      return _;
    }
    class o56 {
      middlewareStack = tb8.constructStack();
      schema;
      static classBuilder() {
        return new HI8();
      }
      resolveMiddlewareWithContext(
        H,
        _,
        q,
        {
          middlewareFn: $,
          clientName: K,
          commandName: O,
          inputFilterSensitiveLog: T,
          outputFilterSensitiveLog: z,
          smithyContext: A,
          additionalContext: f,
          CommandCtor: w,
        },
      ) {
        for (let J of $.bind(this)(w, H, _, q)) this.middlewareStack.use(J);
        let Y = H.concat(this.middlewareStack),
          { logger: D } = _,
          j = {
            logger: D,
            clientName: K,
            commandName: O,
            inputFilterSensitiveLog: T,
            outputFilterSensitiveLog: z,
            [l56.SMITHY_CONTEXT_KEY]: { commandInstance: this, ...A },
            ...f,
          },
          { requestHandler: M } = _;
        return Y.resolve((J) => M.handle(J.request, q || {}), j);
      }
    }
    class HI8 {
      _init = () => {};
      _ep = {};
      _middlewareFn = () => [];
      _commandName = "";
      _clientName = "";
      _additionalContext = {};
      _smithyContext = {};
      _inputFilterSensitiveLog = void 0;
      _outputFilterSensitiveLog = void 0;
      _serializer = null;
      _deserializer = null;
      _operationSchema;
      init(H) {
        this._init = H;
      }
      ep(H) {
        return (this._ep = H), this;
      }
      m(H) {
        return (this._middlewareFn = H), this;
      }
      s(H, _, q = {}) {
        return (this._smithyContext = { service: H, operation: _, ...q }), this;
      }
      c(H = {}) {
        return (this._additionalContext = H), this;
      }
      n(H, _) {
        return (this._clientName = H), (this._commandName = _), this;
      }
      f(H = (q) => q, _ = (q) => q) {
        return (this._inputFilterSensitiveLog = H), (this._outputFilterSensitiveLog = _), this;
      }
      ser(H) {
        return (this._serializer = H), this;
      }
      de(H) {
        return (this._deserializer = H), this;
      }
      sc(H) {
        return (this._operationSchema = H), (this._smithyContext.operationSchema = H), this;
      }
      build() {
        let H = this,
          _;
        return (_ = class extends o56 {
          input;
          static getEndpointParameterInstructions() {
            return H._ep;
          }
          constructor(...[q]) {
            super();
            (this.input = q ?? {}), H._init(this), (this.schema = H._operationSchema);
          }
          resolveMiddleware(q, $, K) {
            let O = H._operationSchema,
              T = O?.[4] ?? O?.input,
              z = O?.[5] ?? O?.output;
            return this.resolveMiddlewareWithContext(q, $, K, {
              CommandCtor: _,
              middlewareFn: H._middlewareFn,
              clientName: H._clientName,
              commandName: H._commandName,
              inputFilterSensitiveLog: H._inputFilterSensitiveLog ?? (O ? i56.bind(null, T) : (A) => A),
              outputFilterSensitiveLog: H._outputFilterSensitiveLog ?? (O ? i56.bind(null, z) : (A) => A),
              smithyContext: H._smithyContext,
              additionalContext: H._additionalContext,
            });
          }
          serialize = H._serializer;
          deserialize = H._deserializer;
        });
      }
    }
    var Jk$ = "***SensitiveInformation***",
      Pk$ = (H, _) => {
        for (let q of Object.keys(H)) {
          let $ = H[q],
            K = async function (T, z, A) {
              let f = new $(T);
              if (typeof z === "function") this.send(f, z);
              else if (typeof A === "function") {
                if (typeof z !== "object") throw Error(`Expected http options but got ${typeof z}`);
                this.send(f, z || {}, A);
              } else return this.send(f, z);
            },
            O = (q[0].toLowerCase() + q.slice(1)).replace(/Command$/, "");
          _.prototype[O] = K;
        }
      };
    class _JH extends Error {
      $fault;
      $response;
      $retryable;
      $metadata;
      constructor(H) {
        super(H.message);
        Object.setPrototypeOf(this, Object.getPrototypeOf(this).constructor.prototype),
          (this.name = H.name),
          (this.$fault = H.$fault),
          (this.$metadata = H.$metadata);
      }
      static isInstance(H) {
        if (!H) return !1;
        let _ = H;
        return (
          _JH.prototype.isPrototypeOf(_) ||
          (Boolean(_.$fault) && Boolean(_.$metadata) && (_.$fault === "client" || _.$fault === "server"))
        );
      }
      static [Symbol.hasInstance](H) {
        if (!H) return !1;
        let _ = H;
        if (this === _JH) return _JH.isInstance(H);
        if (_JH.isInstance(H)) {
          if (_.name && this.name) return this.prototype.isPrototypeOf(H) || _.name === this.name;
          return this.prototype.isPrototypeOf(H);
        }
        return !1;
      }
    }
    var _I8 = (H, _ = {}) => {
        Object.entries(_)
          .filter(([, $]) => $ !== void 0)
          .forEach(([$, K]) => {
            if (H[$] == null || H[$] === "") H[$] = K;
          });
        let q = H.message || H.Message || "UnknownError";
        return (H.message = q), delete H.Message, H;
      },
      qI8 = ({ output: H, parsedBody: _, exceptionCtor: q, errorCode: $ }) => {
        let K = Wk$(H),
          O = K.httpStatusCode ? K.httpStatusCode + "" : void 0,
          T = new q({ name: _?.code || _?.Code || $ || O || "UnknownError", $fault: "client", $metadata: K });
        throw _I8(T, _);
      },
      Xk$ = (H) => {
        return ({ output: _, parsedBody: q, errorCode: $ }) => {
          qI8({ output: _, parsedBody: q, exceptionCtor: H, errorCode: $ });
        };
      },
      Wk$ = (H) => ({
        httpStatusCode: H.statusCode,
        requestId: H.headers["x-amzn-requestid"] ?? H.headers["x-amzn-request-id"] ?? H.headers["x-amz-request-id"],
        extendedRequestId: H.headers["x-amz-id-2"],
        cfId: H.headers["x-amz-cf-id"],
      }),
      Gk$ = (H) => {
        switch (H) {
          case "standard":
            return { retryMode: "standard", connectionTimeout: 3100 };
          case "in-region":
            return { retryMode: "standard", connectionTimeout: 1100 };
          case "cross-region":
            return { retryMode: "standard", connectionTimeout: 3100 };
          case "mobile":
            return { retryMode: "standard", connectionTimeout: 30000 };
          default:
            return {};
        }
      },
      sb8 = !1,
      Rk$ = (H) => {
        if (H && !sb8 && parseInt(H.substring(1, H.indexOf("."))) < 16) sb8 = !0;
      },
      Zk$ = (H) => {
        let _ = [];
        for (let q in l56.AlgorithmId) {
          let $ = l56.AlgorithmId[q];
          if (H[$] === void 0) continue;
          _.push({ algorithmId: () => $, checksumConstructor: () => H[$] });
        }
        return {
          addChecksumAlgorithm(q) {
            _.push(q);
          },
          checksumAlgorithms() {
            return _;
          },
        };
      },
      Lk$ = (H) => {
        let _ = {};
        return (
          H.checksumAlgorithms().forEach((q) => {
            _[q.algorithmId()] = q.checksumConstructor();
          }),
          _
        );
      },
      kk$ = (H) => {
        return {
          setRetryStrategy(_) {
            H.retryStrategy = _;
          },
          retryStrategy() {
            return H.retryStrategy;
          },
        };
      },
      vk$ = (H) => {
        let _ = {};
        return (_.retryStrategy = H.retryStrategy()), _;
      },
      $I8 = (H) => {
        return Object.assign(Zk$(H), kk$(H));
      },
      Nk$ = $I8,
      hk$ = (H) => {
        return Object.assign(Lk$(H), vk$(H));
      },
      yk$ = (H) => (Array.isArray(H) ? H : [H]),
      KI8 = (H) => {
        for (let q in H)
          if (H.hasOwnProperty(q) && H[q]["#text"] !== void 0) H[q] = H[q]["#text"];
          else if (typeof H[q] === "object" && H[q] !== null) H[q] = KI8(H[q]);
        return H;
      },
      Vk$ = (H) => {
        return H != null;
      };
    class OI8 {
      trace() {}
      debug() {}
      info() {}
      warn() {}
      error() {}
    }
    function TI8(H, _, q) {
      let $, K, O;
      if (typeof _ > "u" && typeof q > "u") ($ = {}), (O = H);
      else if ((($ = H), typeof _ === "function")) return (K = _), (O = q), Ck$($, K, O);
      else O = _;
      for (let T of Object.keys(O)) {
        if (!Array.isArray(O[T])) {
          $[T] = O[T];
          continue;
        }
        zI8($, null, O, T);
      }
      return $;
    }
    var Sk$ = (H) => {
        let _ = {};
        for (let [q, $] of Object.entries(H || {})) _[q] = [, $];
        return _;
      },
      Ek$ = (H, _) => {
        let q = {};
        for (let $ in _) zI8(q, H, _, $);
        return q;
      },
      Ck$ = (H, _, q) => {
        return TI8(
          H,
          Object.entries(q).reduce(($, [K, O]) => {
            if (Array.isArray(O)) $[K] = O;
            else if (typeof O === "function") $[K] = [_, O()];
            else $[K] = [_, O];
            return $;
          }, {}),
        );
      },
      zI8 = (H, _, q, $) => {
        if (_ !== null) {
          let T = q[$];
          if (typeof T === "function") T = [, T];
          let [z = bk$, A = Ik$, f = $] = T;
          if ((typeof z === "function" && z(_[f])) || (typeof z !== "function" && !!z)) H[$] = A(_[f]);
          return;
        }
        let [K, O] = q[$];
        if (typeof O === "function") {
          let T,
            z = K === void 0 && (T = O()) != null,
            A = (typeof K === "function" && !!K(void 0)) || (typeof K !== "function" && !!K);
          if (z) H[$] = T;
          else if (A) H[$] = O();
        } else {
          let T = K === void 0 && O != null,
            z = (typeof K === "function" && !!K(O)) || (typeof K !== "function" && !!K);
          if (T || z) H[$] = O;
        }
      },
      bk$ = (H) => H != null,
      Ik$ = (H) => H,
      uk$ = (H) => {
        if (H !== H) return "NaN";
        switch (H) {
          case 1 / 0:
            return "Infinity";
          case -1 / 0:
            return "-Infinity";
          default:
            return H;
        }
      },
      xk$ = (H) => H.toISOString().replace(".000Z", "Z"),
      n56 = (H) => {
        if (H == null) return {};
        if (Array.isArray(H)) return H.filter((_) => _ != null).map(n56);
        if (typeof H === "object") {
          let _ = {};
          for (let q of Object.keys(H)) {
            if (H[q] == null) continue;
            _[q] = n56(H[q]);
          }
          return _;
        }
        return H;
      };
    Object.defineProperty(BT, "collectBody", {
      enumerable: !0,
      get: function () {
        return r56.collectBody;
      },
    });
    Object.defineProperty(BT, "extendedEncodeURIComponent", {
      enumerable: !0,
      get: function () {
        return r56.extendedEncodeURIComponent;
      },
    });
    Object.defineProperty(BT, "resolvedPath", {
      enumerable: !0,
      get: function () {
        return r56.resolvedPath;
      },
    });
    BT.Client = eb8;
    BT.Command = o56;
    BT.NoOpLogger = OI8;
    BT.SENSITIVE_STRING = Jk$;
    BT.ServiceException = _JH;
    BT._json = n56;
    BT.convertMap = Sk$;
    BT.createAggregatedClient = Pk$;
    BT.decorateServiceException = _I8;
    BT.emitWarningIfUnsupportedVersion = Rk$;
    BT.getArrayIfSingleItem = yk$;
    BT.getDefaultClientConfiguration = Nk$;
    BT.getDefaultExtensionConfiguration = $I8;
    BT.getValueFromTextNode = KI8;
    BT.isSerializableHeaderValue = Vk$;
    BT.loadConfigsForDefaultMode = Gk$;
    BT.map = TI8;
    BT.resolveDefaultRuntimeConfig = hk$;
    BT.serializeDateTime = xk$;
    BT.serializeFloat = uk$;
    BT.take = Ek$;
    BT.throwDefaultError = qI8;
    BT.withBaseException = Xk$;
    Object.keys(ab8).forEach(function (H) {
      if (H !== "default" && !Object.prototype.hasOwnProperty.call(BT, H))
        Object.defineProperty(BT, H, {
          enumerable: !0,
          get: function () {
            return ab8[H];
          },
        });
    });
  });
