  var Lw7 = d((X8O, Zw7) => {
    ...
    function kN_(...H) {
      return H.map((q) => z61(q)).join("");
    }
    function A61(H) {
      let _ = /([-a-zA-Z$._][\w$.-]*)/,
        q = { className: "type", begin: /\bi\d+(?=\s|\b)/ },
        $ = { className: "operator", relevance: 0, begin: /=/ },
        K = { className: "punctuation", relevance: 0, begin: /,/ },
        O = {
          className: "number",
          variants: [{ begin: /0[xX][a-fA-F0-9]+/ }, { begin: /-?\d+(?:[.]\d+)?(?:[eE][-+]?\d+(?:[.]\d+)?)?/ }],
          relevance: 0,
        },
        T = { className: "symbol", variants: [{ begin: /^\s*[a-z]+:/ }], relevance: 0 },
        z = { className: "variable", variants: [{ begin: kN_(/%/, _) }, { begin: /%\d+/ }, { begin: /#\d+/ }] },
        A = {
          className: "title",
          variants: [
            { begin: kN_(/@/, _) },
