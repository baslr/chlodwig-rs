  var Ez7 = d((t_O, Sz7) => {
    function Ee4(H) {
      let q = "[a-zA-Z_\\-!.?+*=<>&#'][a-zA-Z_\\-!.?+*=<>&#'0-9/;:]*",
        ...
        O = "[-+]?\\d+(\\.\\d+)?",
        T = { begin: q, relevance: 0 },
        z = { className: "number", begin: "[-+]?\\d+(\\.\\d+)?", relevance: 0 },
        A = H.inherit(H.QUOTE_STRING_MODE, { illegal: null }),
        f = H.COMMENT(";", "$", { relevance: 0 }),
        w = { className: "literal", begin: /\b(true|false|nil)\b/ },
        Y = { begin: "[\\[\\{]", end: "[\\]\\}]" },
        D = { className: "comment", begin: "\\^" + q },
        j = H.COMMENT("\\^\\{", "\\}"),
        M = { className: "symbol", begin: "[:]{1,2}" + q },
        J = { begin: "\\(", end: "\\)" },
        P = { endsWithParent: !0, relevance: 0 },
        X = { keywords: K, className: "name", begin: q, relevance: 0, starts: P },
        R = [J, A, D, j, f, M, Y, z, w, T],
        W = {
          beginKeywords: "def defonce defprotocol defstruct defmulti defmethod defn- defn defmacro deftype defrecord",
          lexemes: q,
          end: '(\\[|#|\\d|"|:|\\{|\\)|\\(|$)',
          contains: [{ className: "title", begin: q, relevance: 0, excludeEnd: !0, endsParent: !0 }].concat(R),
        };
      return (
        (J.contains = [H.COMMENT("comment", ""), W, X, P]),
        (P.contains = R),
        (Y.contains = R),
        (j.contains = [Y]),
        { name: "Clojure", aliases: ["clj"], illegal: /\S/, contains: [J, A, D, j, f, M, Y, z, w] }
      );
    }
    Sz7.exports = Ee4;
  });
