  var Iw7 = d((Z8O, bw7) => {
    ...
    function vN_(...H) {
      return H.map((q) => Cw7(q)).join("");
    }
    function Ew7(...H) {
      return "(" + H.map((q) => Cw7(q)).join("|") + ")";
    }
    function j61(H) {
      let _ = /([2-9]|[1-2]\d|[3][0-5])\^\^/,
        q = /(\w*\.\w+|\w+\.\w*|\w+)/,
        $ = /(\d*\.\d+|\d+\.\d*|\d+)/,
        K = Ew7(vN_(_, q), $),
        z = Ew7(/``[+-]?(\d*\.\d+|\d+\.\d*|\d+)/, /`([+-]?(\d*\.\d+|\d+\.\d*|\d+))?/),
        A = /\*\^[+-]?\d+/,
        w = { className: "number", relevance: 0, begin: vN_(K, Sw7(z), Sw7(A)) },
        Y = /[a-zA-Z$][a-zA-Z0-9$]*/,
        D = new Set(D61),
        j = {
          variants: [
            {
              ...
              begin: Y,
              "on:begin": (Z, k) => {
                if (!D.has(Z[0])) k.ignoreMatch();
              },
            },
            { className: "symbol", relevance: 0, begin: Y },
          ],
        },
        M = { className: "named-character", begin: /\\\[[$a-zA-Z][$a-zA-Z0-9]+\]/ },
        J = { className: "operator", relevance: 0, begin: /[+\-*/,;.:@~=><&|_`'^?!%]+/ },
        P = { className: "pattern", relevance: 0, begin: /([a-zA-Z$][a-zA-Z0-9$]*)?_+([a-zA-Z$][a-zA-Z0-9$]*)?/ },
        X = { className: "slot", relevance: 0, begin: /#[a-zA-Z$][a-zA-Z0-9$]*|#+[0-9]?/ },
        R = { className: "brace", relevance: 0, begin: /[[\](){}]/ },
        W = { className: "message-name", relevance: 0, begin: vN_("::", Y) };
      return {
        name: "Mathematica",
        aliases: ["mma", "wl"],
        classNameAliases: {
        ...
        contains: [H.COMMENT(/\(\*/, /\*\)/, { contains: ["self"] }), P, X, W, j, M, H.QUOTE_STRING_MODE, w, J, R],
      };
    }
    bw7.exports = j61;
  });
