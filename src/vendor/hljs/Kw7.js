  var Kw7 = d((f8O, $w7) => {
    function g_1(H) {
      let K = {
        ...
        O = H.COMMENT("<!--", "-->", { relevance: 0 }),
        T = {
          className: "meta",
          begin: "\\[noprocess\\]",
          starts: { end: "\\[/noprocess\\]", returnEnd: !0, contains: [O] },
        },
        z = { className: "meta", begin: "\\[/noprocess|<\\?(lasso(script)?|=)" },
        A = { className: "symbol", begin: "'[a-zA-Z_][\\w.]*'" },
        f = [
          H.C_LINE_COMMENT_MODE,
          H.C_BLOCK_COMMENT_MODE,
          H.inherit(H.C_NUMBER_MODE, { begin: H.C_NUMBER_RE + "|(-?infinity|NaN)\\b" }),
          ...
          { begin: /(->|\.)\s*/, relevance: 0, contains: [A] },
          {
            className: "class",
            beginKeywords: "define",
            returnEnd: !0,
            end: "\\(|=>",
            contains: [H.inherit(H.TITLE_MODE, { begin: "[a-zA-Z_][\\w.]*(=(?!>))?|[-+*/%](?!>)" })],
          },
        ];
      return {
        name: "Lasso",
