  var nE8 = d((st) => {
    ...
    class QE8 {
      ...
      kind;
      values;
      constructor({ name: H, kind: _ = AR$.FieldPosition.HEADER, values: q = [] }) {
    ...
    class CO_ {
      ...
      static clone(H) {
        let _ = new CO_({ ...H, headers: { ...H.headers } });
        if (_.query) _.query = YR$(_.query);
        return _;
      }
      static isInstance(H) {
        if (!H) return !1;
    ...
    st.HttpRequest = CO_;
    st.HttpResponse = iE8;
    st.getHttpHandlerExtensionConfiguration = fR$;
    st.isValidHostname = DR$;
    st.resolveHttpHandlerRuntimeConfig = wR$;
  });
