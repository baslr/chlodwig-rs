  var Xh8 = d((Ft) => {
    ...
    class Mh8 {
      ...
      kind;
      values;
      constructor({ name: H, kind: _ = OP$.FieldPosition.HEADER, values: q = [] }) {
    ...
    class B3_ {
      ...
      static clone(H) {
        let _ = new B3_({ ...H, headers: { ...H.headers } });
        if (_.query) _.query = AP$(_.query);
        return _;
      }
      static isInstance(H) {
        if (!H) return !1;
    ...
    Ft.HttpRequest = B3_;
    Ft.HttpResponse = Ph8;
    Ft.getHttpHandlerExtensionConfiguration = TP$;
    Ft.isValidHostname = fP$;
    Ft.resolveHttpHandlerRuntimeConfig = zP$;
  });
