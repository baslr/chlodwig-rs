...
fn test_cursor_up_simple_newline() {
    ...
    app.input = "hello\nworld".to_string();
    app.cursor = 9; // 'l' in "world" (char 9: h-e-l-l-o-\n-w-o-r-l)
    ...
    assert_eq!(app.cursor, 3, "cursor should move up to same column on previous line");
}
...
fn test_cursor_up_soft_wrap() {
    ...
    app.cursor = 13; // char 13 → row 1, col 3
    app.move_cursor_up(10);
    // Should move to row 0, col 3 → char 3
    assert_eq!(app.cursor, 3, "cursor should move up on soft-wrapped line");
}
...
fn test_cursor_down_simple_newline() {
    let mut app = App::new("test".into());
    app.input = "hello\nworld".to_string();
    app.cursor = 3; // 'l' in "hello" (row 0, col 3)
    app.move_cursor_down(80);
    // Should move to row 1, col 3 → char 9 ('l' in "world": \n is char 5, w=6,o=7,r=8,l=9)
    assert_eq!(app.cursor, 9, "cursor should move down to same column on next line");
}
...
fn test_cursor_down_clamps_to_shorter_line() {
    ...
    app.cursor = 7; // 'o' in "world" → row 0, col 7
    app.move_cursor_down(80);
    // Row 1 = "hi" (2 chars), col 7 is past end → clamp to end of "hi"
    // "hello world\nhi" — \n is char 11, h=12, i=13, end=14
    assert_eq!(app.cursor, 14, "cursor should clamp to end of shorter line");
}
...
fn test_cursor_down_soft_wrap() {
    let mut app = App::new("test".into());
    app.input = "a".repeat(20);
    app.cursor = 3; // row 0, col 3
    app.move_cursor_down(10);
    // Should move to row 1, col 3 → char 13
    assert_eq!(app.cursor, 13, "cursor should move down on soft-wrapped line");
}
...
fn test_cursor_up_down_three_lines() {
    ...
    app.input = "abc\ndef\nghi".to_string();
    // Chars: a=0 b=1 c=2 \n=3 d=4 e=5 f=6 \n=7 g=8 h=9 i=10
    app.cursor = 9; // 'h' on line 3, col 1
    app.move_cursor_up(80);
    assert_eq!(app.cursor, 5, "should move from line 3 to line 2, col 1 → 'e'");
    app.move_cursor_up(80);
    assert_eq!(app.cursor, 1, "should move from line 2 to line 1, col 1 → 'b'");
    app.move_cursor_down(80);
    assert_eq!(app.cursor, 5, "should move back to line 2, col 1 → 'e'");
    app.move_cursor_down(80);
    assert_eq!(app.cursor, 9, "should move back to line 3, col 1 → 'h'");
}
...
fn test_cursor_up_down_restores_clamped() {
    ...
    // Chars: h=0..!=13 \n=14 h=15 i=16 \n=17 a=18..k=28
    app.cursor = 25; // 'h' in "abcdefghijk" → row 2, col 7
    app.move_cursor_up(80); // → row 1, col clamped to end of "hi" → char 17 (end of "hi" line)
    assert_eq!(app.cursor, 17, "should clamp to end of short line");
    ...
    assert_eq!(app.cursor, 20, "should move to col 2 on line 2 (no sticky column)");
}
...
fn test_cursor_up_cjk_display_width() {
    ...
    // Chars: a=0..f=5 \n=6 日=7 本=8 語=9
    app.cursor = 8; // '本' on line 1, display col 2
    app.move_cursor_up(80);
    // Display col 2 on line 0 → char 2 ('c')
    assert_eq!(app.cursor, 2, "should land on char at display col 2");
}
