...
/// Returns the log directory (`~/.chlodwig-rs/`), creating it if necessary.
...
/// Returns a timestamped log file path, e.g. `~/.chlodwig-rs/debug_2026-04-04_14-30-15.log`.
///
...
mod tests {
    ...
    #[test]
    fn test_timestamped_log_path_is_in_chlodwig_rs_dir() {
        let path = timestamped_log_path("debug");
        let parent = path.parent().unwrap();
        assert!(
            parent.ends_with(".chlodwig-rs"),
            "Should be in .chlodwig-rs dir: {}",
            parent.display()
        );
    }
    ...
    #[test]
    fn test_log_dir_ends_with_chlodwig_rs() {
        let dir = log_dir();
        assert!(
            dir.ends_with(".chlodwig-rs"),
            "log_dir should end with .chlodwig-rs: {}",
            dir.display()
        );
    }
}
