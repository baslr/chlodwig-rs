...
pub(crate) fn write_crash_to_static_buf(signal_name: &str) {
    ...
    // --- Header ---
    crash_buf_write(b"claude-rs SIGNAL CRASH REPORT\n");
    crash_buf_write(b"==============================\n");
...
/// 2. Writes a crash report with backtrace + App state to ~/.claude-rs/crash.log
...
fn install_panic_hook() {
    ...
    std::panic::set_hook(Box::new(move |info| {
        ...
        // 5. Write to file
        let log_path = claude_core::timestamped_log_path("crash");
...
pub async fn run_tui_with_permissions(
    ...
    let permission_prompter: std::sync::Arc<dyn PermissionPrompter> = if bypass_permissions {
        std::sync::Arc::new(claude_core::AutoApprovePrompter)
    } else {
    ...
    loop {
        if needs_redraw {
            ...
            if let Err(e) = terminal.draw(|f| ui(f, &app)) {
                ...
                eprintln!("\nclaude-rs: terminal.draw() error: {e}");
                            ...
                            && app.pending_permission.is_none() =>
                    {
                        ...
                        if trimmed == "/compact" || trimmed.starts_with("/compact ") {
                            ...
                            tokio::spawn(async move {
                                ...
                                if let Err(e) = claude_core::compact_conversation(
                                    &mut state_guard,
                                    api_ref,
                                    &tx,
                        ...
                        let needs_compact = app.total_input_tokens > claude_core::AUTO_COMPACT_THRESHOLD;
                        ...
                        tokio::spawn(async move {
                            ...
                            state_guard.messages.push(claude_core::Message {
                                role: claude_core::Role::User,
                                content: vec![claude_core::ContentBlock::Text { text: prompt }],
                            });
                            if let Err(e) = claude_core::run_turn(
                                &mut state_guard,
                                api_ref,
                                perm_clone.as_ref(),
                                &tx,
