...
fn test_sys_prompt_lines_built_from_blocks() {
    ...
    app.system_prompt_blocks = vec![
        claude_core::SystemBlock::cached("You are Claude."),
        claude_core::SystemBlock::text("Git branch: main"),
    ];
...
fn test_sys_prompt_second_block_no_cached_tag() {
    let mut app = App::new("test".into());
    app.system_prompt_blocks = vec![
        claude_core::SystemBlock::text("Dynamic context"),
    ];
    app.rebuild_sys_prompt_lines();
...
fn test_tab_scroll_down_sys_prompt() {
    let mut app = App::new("test".into());
    app.active_tab = 1;
    app.system_prompt_blocks = vec![claude_core::SystemBlock::text(
        &"line\n".repeat(200),
    )];
    app.rebuild_sys_prompt_lines();
...
fn test_tab_scroll_down_clamps_at_max() {
    let mut app = App::new("test".into());
    app.active_tab = 1;
    app.system_prompt_blocks = vec![claude_core::SystemBlock::text("short")];
    app.rebuild_sys_prompt_lines();
