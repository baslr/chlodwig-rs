...
#[command(
    name = "claude-rs",
    about = "Claude Code CLI — Rust PoC",
    version,
    long_about = "A proof-of-concept Rust implementation of the Claude Code CLI.\nStreams responses from the Anthropic Messages API with tool execution support."
)]
struct Cli {
...
async fn main() -> Result<()> {
    ...
    // Initialize tracing
    // Log to ~/.claude-rs/debug_YYYY-MM-DD_HH-MM-SS.log
    let debug_log_path = claude_core::timestamped_log_path("debug");
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::from_default_env()
                .add_directive("claude_core=debug".parse().unwrap())
                .add_directive("claude_api=debug".parse().unwrap())
                .add_directive("claude_tools=debug".parse().unwrap())
                .add_directive("claude_tui=debug".parse().unwrap())
    ...
    tracing::info!("claude-rs starting, log at {:?}", debug_log_path);
    ...
    let tools = claude_tools::builtin_tools();
    ...
    if let Some(ref prompt) = cli.print_mode {
        ...
        claude_tui::run_tui_with_permissions(state, Box::new(client), cli.dangerously_skip_permissions).await
    }
}
...
async fn run_headless(
    ...
    let loop_result = {
        ...
        let result =
            claude_core::run_turn(&mut state, api_client, permission_ref, &tx).await;
